...
func TestRootSyncReconcileWithInvalidCACertSecret(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			rootsync.SetStalled(wantRs, "Validation", validate.MissingKeyInCACertSecret(CACertSecretKey, caCertSecret))
...
func TestRootSyncReconcilerWithGithubApp(t *testing.T) {
	// Mock out parseDeployment for testing.
	parseDeployment = parsedDeployment

	secretRef := "my-secret"
	rs := rootSyncWithGit(rootsyncName,
		rootsyncSecretType(configsync.AuthGithubApp),
	)
	reqNamespacedName := namespacedName(rs.Name, rs.Namespace)
	secretObject := k8sobjects.SecretObject(secretRef, core.Namespace(rs.Namespace))
	fakeClient, fakeDynamicClient, testReconciler := setupRootReconciler(t, rs, secretObject)
...
func TestReconcile_DisableMonitoring(t *testing.T) {
	...
	t.Run("DISABLE_MONITORING=true", func(t *testing.T) {
		t.Setenv("DISABLE_MONITORING", "true")
	...
	t.Run("DISABLE_MONITORING=false", func(t *testing.T) {
		t.Setenv("DISABLE_MONITORING", "false")
