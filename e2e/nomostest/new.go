...
func newOptStruct(testName, tmpDir string, ntOptions ...ntopts.Opt) *ntopts.New {
	...
	optsStruct := &ntopts.New{
		...
		MultiRepo: ntopts.MultiRepo{
			...
			// Default to foreground to clean up test resources
			DeletionPropagationPolicy: ptr.To(metadata.DeletionPropagationPolicyForeground),
		},
	}
...
func SharedTestEnv(t nomostesting.NTB, opts *ntopts.New) *NT {
	...
	nt := &NT{
		...
		deletionPropagationPolicy: opts.DeletionPropagationPolicy,
	}
